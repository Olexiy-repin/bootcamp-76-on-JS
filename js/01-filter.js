...
//   const filteredCarsByPrice = structuredClone(cars).filter((el, idx, arr) => {
...
// ~ Скорочена версія
// const filterByPrice = (cars, threshold) => structuredClone(cars).filter(car => car.price <= threshold);
...
//   const carsWithDiscount = structuredClone(cars).filter((el, idx, arr) => {
//     return el.onSale;
//   });
...
// ~ Скорочена версія
// const getCarsWithDiscount = cars => structuredClone(cars).filter(car => car.onSale);
...
//   const carsWithType = structuredClone(cars).filter((el, idx, arr) => {
...
//   return structuredClone(cars).filter(car => car.type.toLowerCase() === type);
// };
